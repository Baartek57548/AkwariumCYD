...
bool Win32Window::Create(const std::wstring& title,
  ...
  if (GetMonitorInfo(monitor, &monitor_info)) {
    ...
    window_x = std::clamp(work_area.left + window_x, work_area.left,
                          work_area.right - window_width);
    window_y = std::clamp(work_area.top + window_y, work_area.top,
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    case WM_GETMINMAXINFO: {
      ...
      int minimum_width = Scale(480, scale_factor);
      int minimum_height = Scale(420, scale_factor);
      MONITORINFO monitor_info{};
      monitor_info.cbSize = sizeof(monitor_info);
